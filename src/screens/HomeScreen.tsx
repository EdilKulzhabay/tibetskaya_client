...
const HomeScreen: React.FC = () => {
  ...
  const getLastOrder = async (clientIdOverride?: string) => {
    ...
    if (user && clientId && lastOrder) {
        ...
        if (user?.paymentMethod === 'balance') {
          if (
            ...
            const deficit = Math.max(
              0,
              Math.ceil(lastOrder.sum - user.balance),
            );
            const afterTopUp = () => {
      ...
      try {
        res = await apiService.addOrder(
          clientId,
          resolveRepeatOrderAddress(user, lastOrder.address),
          lastOrder.products,
          [],
          orderDateObject,
          paymentValue,
        ...
        if (message === 'Недостаточно средств на балансе') {
          ...
          const deficit = Math.max(
            0,
            Math.ceil(lastOrder.sum - Number(user.balance || 0)),
          );
          const afterTopUp = () => {
            const ymd = repeatOrderDeliveryYmdRef.current;
            if (!ymd || !clientId || !lastOrder) return;
  ...
  const onRepeatDateSelectedFromModal = (deliveryYmd: string) => {
    setRepeatDateModalVisible(false);
    if (!user || !getClientMongoId(user) || !lastOrder) return;
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          ...
          {lastOrderLoading ? (
            <RepeatOrderSkeleton roundedBottom={!showRepairMasterBlock} />
          ) : lastOrder ? (
            <TouchableOpacity
                  ...
                  <View>
                    ...
                    <Text
                      ...
                      {lastOrder.products.b12 > 0
                        ? `${lastOrder.products.b12}x Вода 12,5 л`
                        : ''}
                      {lastOrder.products.b12 > 0 && lastOrder.products.b19 > 0
                        ? `\n`
                        : ''}
                      {lastOrder.products.b19 > 0
                        ? `${lastOrder.products.b19}x Вода 18,9л.`
                        : ''}
                    </Text>
                  </View>
                  <View
              ...
              <View
                ...
                <Text
                  style={{color: '#DC1818', fontSize: 14, fontWeight: '600'}}>
                  Повторить
                </Text>
              </View>
            </TouchableOpacity>
          ...
          {orders.length > 0 && (
            <View style={styles.activeOrdersContainer}>
              <View style={styles.activeOrdersTitle}>
                <Text style={styles.activeOrdersTitleText}>
          ...
          <Products
            navigation={navigation}
            price19={user?.price19 || 1300}
            price12={user?.price12 || 900}
          />
          {/* <Marketplace /> */}
          <Image
