...
export const TopUpBalanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const topUpSuccessActionRef = useRef<((updatedUser?: User | null) => void) | null>(null);
    ...
    const armTopUpBackdrop = useCallback((ref: React.MutableRefObject<boolean>) => {
        ref.current = false;
    ...
    const checkPendingKaspiInvoice = useCallback(async () => {
        ...
        try {
            ...
            if (status === 'paid') {
                ...
                const secondFresh = await refreshUserData();
                notifyTopUpSuccess((secondFresh ?? firstFresh ?? null) as User | null);
            } else if (['cancelled', 'expired', 'error', 'refunded'].includes(status)) {
                pendingKaspiInvoiceIdRef.current = null;
                kaspiStatusCheckAttemptsRef.current = 0;
            } else if (['pending', 'processing', ''].includes(status) && kaspiStatusCheckAttemptsRef.current < 8) {
    ...
    const handleGenerateInvoice = useCallback(async () => {
        ...
        const profileUser =
            ...
            ? ({ ...user, ...fresh } as User)
            : ((fresh ?? user) as User | null);
        const mail = profileUser?.mail?.trim();
        if (!mail) {
            Alert.alert('Ошибка', 'Не удалось загрузить профиль. Проверьте соединение и попробуйте снова.');
            return;
        }
        const res = await apiService.generateInvoicePdf(mail, q19, q12);
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        {topUpOptions.showCashPayment ? (
                            <View style={{
                                ...
                                <View style={{ marginLeft: 16, maxWidth: "90%" }}>
                                    <Text style={{ fontSize: 22, fontWeight: '500', color: '#000', textAlign: 'center' }}>Не хватает {topUpOptions.title}</Text>
                                    <Text style={{ fontSize: 13, fontWeight: '400', color: '#4A5565', textAlign: 'center' }}>Чтобы оформить заказ, пополните баланс на необходимую сумму.</Text>
                                </View>
                            </View>
                        ) : null}
                        <View>
                            <Text style={{ fontSize: 14, fontWeight: '500', color: '#101828', marginTop: 16 }}>Способы пополнения</Text>
                        </View>
                        <TouchableOpacity
                            style={styles.bottomSheetPayBtn}
                            onPress={() => openTopUpAmountSheet('kaspi')}
                        >
                            <View style={{ flexShrink: 0, width: 48, height: 48, justifyContent: 'center', alignItems: 'center', borderRadius: 8 }}>
                                <Image source={require('../assets/kaspi.png')} style={{ width: 48, height: 48 }} />
                            </View>
                            <View style={{ marginLeft: 12, maxWidth: "65%" }}>
                                <Text style={{ fontSize: 16, fontWeight: '500', color: '#101828' }}>Пополнить через Kaspi</Text>
                                <Text style={{ fontSize: 14, fontWeight: '400', color: '#4A5565' }}>На сумму {topUpOptions.title}</Text>
                            </View>
                            <View style={{ flexShrink: 0, width: 20, height: 20, marginLeft: "auto" }}>
                        ...
                        >
                            <View style={{ flexShrink: 0, width: 48, height: 48, justifyContent: 'center', alignItems: 'center', borderRadius: 8, backgroundColor: "#EFF6FF" }}>
                                <Image source={require('../assets/card.png')} style={{ width: 24, height: 24 }} />
                            </View>
                            <View style={{ marginLeft: 12, maxWidth: "65%" }}>
                                <Text style={{ fontSize: 16, fontWeight: '500', color: '#101828' }}>Пополнить банковской картой</Text>
                                <Text style={{ fontSize: 14, fontWeight: '400', color: '#4A5565' }}>Visa · Mastercard · Apple Pay · Google Pay</Text>
                            </View>
                            <View style={{ flexShrink: 0, width: 20, height: 20, marginLeft: "auto" }}>
                                <Image source={require('../assets/arrowRight.png')} style={{ width: 20, height: 20 }} />
                            </View>
                        ...
                        {topUpOptions.showCashPayment ? (
                            <TouchableOpacity style={styles.bottomSheetPayBtn} onPress={handleCashPayment}>
                                <View style={{ flexShrink: 0, width: 48, height: 48, justifyContent: 'center', alignItems: 'center', borderRadius: 8, backgroundColor: "#F0FDF4" }}>
                                    <Image source={require('../assets/fakt.png')} style={{ width: 24, height: 24 }} />
                                </View>
                                <View style={{ marginLeft: 12, maxWidth: "65%" }}>
                                    <Text style={{ fontSize: 16, fontWeight: '500', color: '#101828' }}>Наличными курьеру</Text>
                                    <Text style={{ fontSize: 14, fontWeight: '400', color: '#4A5565' }}>QR, переводом или наличными</Text>
                                </View>
                                <View style={{ flexShrink: 0, width: 20, height: 20, marginLeft: "auto" }}>
                                    <Image source={require('../assets/arrowRight.png')} style={{ width: 20, height: 20 }} />
                                </View>
                        ...
                        <View style={{
                            ...
                            <Image source={require('../assets/guard.png')} style={{ width: 20, height: 20 }} />
                            <Text style={{ fontSize: 12, fontWeight: '500', color: '#0D542B', marginLeft: 8 }}>Безопасно и конфиденциально</Text>
                        </View>
                ...
                >
                    ...
                    >
                        ...
                        <Text style={styles.bottomSheetTitle}>
                            {topUpMethod === 'kaspi' ? 'Пополнение через Kaspi' : 'Пополнение банковской картой'}
                        </Text>
                        ...
                        <TouchableOpacity style={{
                            ...
                            alignItems: 'center',
                            marginBottom: 10,
                        }} disabled={topUpSubmitting} onPress={() => {
