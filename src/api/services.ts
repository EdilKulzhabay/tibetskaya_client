...
export const apiService = {
  getData: async (clientId: string) => {
    try {
      const response = await api.post('/getClientDataMobile', {clientId});
  ...
  /** Код регистрации отправляется в WhatsApp на указанный телефон */
  ...
  updateData: async (clientId: string, field: string, value: any) => {
    try {
      const response = await api.post(`/updateClientDataMobile`, {
        clientId,
        field,
        value,
      });
  ...
  generateInvoicePdf: async (
    ...
    try {
      const response = await api.post('/generateInvoicePdfMobile', {
        clientId,
        qty19,
        qty12,
      });
  ...
  createKaspiQrInvoice: async (
    amount: number,
    clientId: string,
    pendingOrderDraft?: unknown,
  ) => {
    try {
      const response = await api.post('/api/apipay/qr/create', {
        amount,
        clientId,
        pendingOrderDraft,
      });
  ...
  getActiveOrders: async (clientId: string) => {
    try {
      const response = await api.post('/getActiveOrdersMobile', {clientId});
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  getOrders: async (clientId: string) => {
    try {
      const response = await api.post('/getClientOrdersMobile', {clientId});
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  deleteAccount: async (clientId: string) => {
    try {
      const response = await api.post('/deleteClientMobile', {clientId});
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  sendSupportMessage: async (clientId: string, message: SupportMessage) => {
    try {
      const response = await api.post('/sendSupportMessage', {
        clientId,
        message,
      });
      return response.data;
  ...
  getSupportMessages: async (clientId: string) => {
    try {
      const response = await api.post('/getSupportMessages', {clientId});
      return response.data;
    } catch (error) {
      return {
        success: false,
  ...
  updateOrderData: async (orderId: string, field: string, value: any) => {
    try {
      const response = await api.post('/updateOrderDataMobile', {
        orderId,
        field,
        value,
      });
  ...
  getLastOrder: async (clientId: string) => {
    try {
      const response = await api.post('/getLastOrderMobile', {clientId});
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  requestMasterCall: async (payload: {
    fullName: string;
    phone: string;
    mail: string;
  }) => {
  ...
  sendMailForgotPassword: async (identifier: {
    mail?: string;
    phone?: string;
  }) => {
    try {
      const response = await api.post('/sendMailForgotPassword', identifier);
      return response.data;
    } catch (error) {
      return {
        success: false,
  ...
  codeConfirmForgotPassword: async (
    identifier: {mail?: string; phone?: string},
    code: string,
  ) => {
    try {
      const response = await api.post('/codeConfirmForgotPassword', {
        ...identifier,
        code,
      });
      return response.data;
  ...
  updateForgottenPassword: async (
    ...
    try {
      const response = await api.post('/updateForgottenPassword', {
        ...identifier,
        password,
      });
      return response.data;
  ...
  chargeSavedCard: async (clientId: string, amount: number) => {
    try {
      const response = await api.post('/api/payment/charge-saved-card', {
        clientId,
        amount,
      });
      return response.data;
  ...
  getWidgetConfig: async (
    ...
    try {
      console.log('[getWidgetConfig] Запрос:', {
        userId,
        amount,
        hasEmail: !!email,
        hasPhone: !!phone,
      });
      const response = await api.post('/api/payment/widget-config', {
        userId,
        amount,
        email,
        phone,
      ...
      const serverMessage =
        ...
        'Не удалось получить конфигурацию платежа';
      const debugMessage = error?.response?.data?.debug;
      return {
        success: false,
        message: debugMessage
          ? `${serverMessage}: ${debugMessage}`
          : serverMessage,
      };
    }
  },
